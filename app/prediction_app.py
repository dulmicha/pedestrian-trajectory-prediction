...
class PedestrianTrajectoryPredictionApp:
    def __init__(self, root, video_path=None, predicting_model_path="models/model_td_20lstm.h5"):
        self.root = root
        self.root.title("Pedestrian trajectory prediction")

        # Variables for video and plotting
        self.video_source = "data/default_video.mp4" if video_path is None else video_path
        ...
        self.predictor = Predictor(predicting_model_path, self.cap)
    ...
    def run(video_path, predicting_model_path):
        root = tk.Tk()
        app = PedestrianTrajectoryPredictionApp(root)
